...
namespace
{
  ...
  const std::string CREDENTIAL_FILTER = ELEMENT_PREFERENCES_KEY_PREFIX + '*';
  ...
  std::wstring Utf8ToWide(const std::string& input) {
      ...
      MultiByteToWideChar(
          CP_UTF8, 0, input.c_str(), -1, buffer.data(), required);
  ...
  std::string WideToUtf8(const std::wstring& input) {
      ...
      WideCharToMultiByte(
          CP_UTF8, 0, input.c_str(), -1, buffer.data(), required, nullptr, nullptr);
...
#pragma warning(pop)
  ...
  flutter::EncodableMap FlutterSecureStorageWindowsPlugin::ReadAll()
  {
    ...
    const std::wstring credential_filter = Utf8ToWide(CREDENTIAL_FILTER);
    ...
    {
      auto pcred = pcreds[i];
      std::string target_name = WideToUtf8(std::wstring(pcred->TargetName));
      auto val = std::string((char*)pcred->CredentialBlob);
